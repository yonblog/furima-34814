class PersonalPurchasing
  ...
  validates :postal_code, format: {with: /\A[0-9]{3}-[0-9]{4}\z/, message: "is invalid. Please enter the correct number and hyphen (-)"}, allow_blank: true
  validates :phone_number, format: {with: /\A\d{10}$|^\d{11}\z/, message: "Enter a 10- or 11-digit number"}, allow_blank: true
